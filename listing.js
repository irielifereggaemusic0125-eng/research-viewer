...
/* ── AIに渡すプロンプトを組み立てる ──────────────────────────
   このアプリは決定的なテンプレ埋めしかできない（自由文から意味を汲み取れない）。
   文章を書く部分はAIの仕事so、ルール・材料・制約を1つのプロンプトにまとめて渡す。 */
function buildPrompt(){
  const A=v('tpl')==='A', useB=ck('useBanshee'), useT=ck('useTag');
  const g=grossJP(), ind=indicator(g);
  const facts=[['ブランド英',v('bEn')],['ブランドカナ',v('bJa')],['商品名',v('name')],['型番/品番',v('model')],
    ['デザイナー',v('designer')],['素材',v('mat')],['カラー',v('color')],['サイズ',v('size')],
    ['実測',v('meas')],['重量',v('wt')],['付属品',v('acc')],['参考定価',v('retail')],
    ['状態メモ',v('cond')],['出品価格','¥'+(n('price')||0).toLocaleString()]]
    .filter(x=>x[1]).map(x=>`- ${x[0]}: ${x[1]}`).join('\n');
  const p = `あなたはBansheeのメルカリ出品文ライターです。以下の材料から、タイトルと商品説明を書いてください。

# 材料（分かっている事実）
${facts||'（フォーム未入力）'}
${v('srcMemo')?`\n# メモ・元の説明文\n${v('srcMemo')}\n`:''}
# 出力するもの
1) 商品タイトル（40字以内・全角半角問わず40字）
   構成: ブランド英 + ブランドカナ + モデル名 + 型番 + サイズ + カラー + 特徴語
   検索されやすい語を優先。実ブランドと違うブランド名は入れない（誤認を招く表現so禁止）。
2) 商品説明（**900〜1000字**・1000字を絶対に超えない）

# 説明文の構成（この順・罫線は「${RULE}」をそのまま使う）
ご覧いただきありがとうございます(^^)
${RULE}
ブランド英 ブランドカナ / 商品名 / デザイナー・素材（改行で）
${RULE}
紹介文（2〜4文。特徴・魅力・使用シーン。**材料から膨らませてよいが、断定できないことは書かない**）
（空行）英語1行サマリー
${A&&useT?'（空行）#全商品Banshee\n':''}${RULE}
【商品詳細】
${RULE}
ブランド／デザイナー／モデル／品番／サイズ／素材／全長／重量／カラー／付属品／参考定価 を「項目：値」で列挙（分かるものだけ）
${RULE}
【状態／Condition】
${RULE}
状態の説明（正直に。誇張しない）
（空行）英語1行
${A&&useB?`${RULE}\n【Bansheeについて】\n${RULE}\n当ショップの商品は全て正規品です。古物商許可のもと、ブランド品を専門に扱う大手古物市場よりお譲りいただいております。\n（空行）古物商許可証：愛知県公安委員会 第542652509100号\n`:''}${ind}
ご不明な点はお気軽にコメントください(^^)
（空行）ハッシュタグ8〜12個

# 厳守ルール
- 絵文字は (^^) ☆ ♪ のみ。他は一切使わない
- 「仕入れております」ではなく「お譲りいただいております」
- 「存じます」は使わない（カジュアル敬語）
- ランク表記（AB/B等）は書かない。状態は文章で表す
- 実測値は「約○cm（実測）」と明記${ck('isAppa')?'\n- アパレルso「※平置き実寸のため、多少の誤差はご了承ください」を入れる':''}
- 製造年・年代・適合車種など**断定できない情報は書かない**
- **最後の罫線は「${ind}」をそのまま使う**（本数に意味があるso増減も置換もしない）
- 他の罫線は全て「${RULE}」で統一

# 出力形式
タイトルと説明文だけを返す。前置き・解説は不要。`;
  const el=$('outP'); if(el) el.value=p;
}
